...
```{r}
mean1 <- mean(my_data$debt_to_income_ratio)
```
Null Hypothesis (H0): The true mean for debt to income ratio is equal to `r mean1`

Alternative Hypothesis (H1): The true mean for debt to income ratio is equal to `r mean1`

```{r}
t_test = t.test(x=my_data$debt_to_income_ratio, mu=mean1, alternative="two.sided")
t_test
```

The p-value from the t-test is equal to 1, indicating that we fail to reject the null hypothesis. Therefore, the true mean of the population for debt to income ration is `r mean1`.



### Goodness-of-fit Test
```{r}
hist(my_data$number_of_late_payments, xlab = "Number of late payments", main = "Histogram of number of late payments", col = "#006B34")
```

```{r}
freq = table(my_data$number_of_late_payments)

dt(x = freq, df = length(freq))
