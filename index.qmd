...
For `poor credit ratings`, there is an insignificant number of engineers and managers. Clerks have a significanctly higher median debt to income ratio than all the other occupations. 
...
data <- data.frame(
  late_payments = discrete_late_payments,
...
The analyses suggest that a high paying occupation protects an individual from having a poor credit rating. It has also been confirmed that the true average for debt to income ratio is `r mean1`.
...
2.  **Address Variability in Sunlight**: 

3.  **Optimize Daily Production**: 
